...
class Configuration(object):
    ...
    def initialize(cls, load_interface=True):
        ...
        cls.pmkid_timeout = 60  # V3: Increased to 60s (was 30s) for better PMKID capture rate

        # V3: WPA3/SAE variables
        cls.use_wpa3 = True   # Enable WPA3/SAE attack module
        cls.wpa3_timeout = 120  # Seconds to wait for WPA3 SAE capture

        # V3: PMF (Protected Management Frames) handling
        cls.pmf_skip_deauth = True  # Skip deauth if target has 802.11w PMF (default: True)

        # V3: Report generation
        cls.report_format = None   # None, 'html', 'json', 'md'
        cls.report_path = 'wifite_report'  # Output path prefix (no extension)
    ...
    def load_from_arguments(cls):
        ...
        cls.parse_wpa3_args(args)
        cls.parse_report_args(args)
    ...
    @classmethod
    def parse_wpa3_args(cls, args):
        '''V3: Parse WPA3/SAE options.'''
        if hasattr(args, 'use_wpa3') and args.use_wpa3 is not None:
            cls.use_wpa3 = args.use_wpa3

        if hasattr(args, 'wpa3_timeout') and args.wpa3_timeout:
            cls.wpa3_timeout = args.wpa3_timeout
            Color.pl('{+} {C}option:{W} WPA3 capture timeout: {G}%ds{W}' % args.wpa3_timeout)

    @classmethod
    def parse_report_args(cls, args):
        '''V3: Parse report generation options.'''
        if hasattr(args, 'report_format') and args.report_format:
            cls.report_format = args.report_format
    ...
    def delete_temp(cls):
        ...
        if os.path.exists(cls.temp_dir):
            ...
            for f in os.listdir(cls.temp_dir):
                filepath = os.path.join(cls.temp_dir, f)
                if os.path.isfile(filepath):
                    try:
                        file_size = os.path.getsize(filepath)
                        if file_size > 0:
                            with open(filepath, 'ba+', buffering=0) as temp_file:
                                for _ in range(3):
                                    temp_file.seek(0)
                                    temp_file.write(secrets.token_bytes(file_size))
                                    temp_file.flush()
