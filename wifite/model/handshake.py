...
# V3: Pyrit replaced with Scapy (pyrit is unmaintained / Python3 incompatible)
...
class Handshake(object):
    ...
    def divine_bssid_and_essid(self):
        ...
        if len(pairs) == 0:
            pairs = self.scapy_handshakes() # Find bssid/essid pairs that have handshakes in Scapy
    ...
    def has_handshake(self):
        ...
        # V3: tshark (primary) + Scapy (fallback) — pyrit removed
        if len(self.tshark_handshakes()) > 0:   return True
        if len(self.scapy_handshakes()) > 0:     return True
    ...
    def scapy_handshakes(self):
        '''
        V3: Validates handshake using Scapy as fallback when tshark is unavailable.
        Replaces pyrit_handshakes() (pyrit is unmaintained).

        Returns list[tuple] of (bssid, None) for each valid handshake found.
        Returns empty list if Scapy is unavailable or no handshake found.
        '''
        try:
            from scapy.all import rdpcap, EAPOL, Dot11
        except ImportError:
            return []  # Scapy not installed; skip silently

        try:
            packets = rdpcap(self.capfile)
        except Exception:
            return []

        # Extract EAPOL packets, optionally filtered by BSSID
        eapol_msgs = []
        for pkt in packets:
            if EAPOL not in pkt:
                continue
            if Dot11 in pkt:
                # Check BSSID matches if we know it
                addr1 = getattr(pkt[Dot11], 'addr1', '')
                addr2 = getattr(pkt[Dot11], 'addr2', '')
                addr3 = getattr(pkt[Dot11], 'addr3', '')
                if self.bssid:
                    bssid_lc = self.bssid.lower()
                    if not any(a and a.lower() == bssid_lc for a in (addr1, addr2, addr3)):
                        continue
            eapol_msgs.append(pkt)

        # A valid 4-way handshake requires at least 4 EAPOL frames
        if len(eapol_msgs) >= 4:
    ...
    def analyze(self):
        ...
        V3: Uses Scapy instead of pyrit.
        '''
        self.divine_bssid_and_essid()
        ...
        # V3: Scapy replaces pyrit
        Handshake.print_pairs(self.scapy_handshakes(),   self.capfile, 'scapy')
