...
class Target(object):
    '''
        ...
        V3: Added pmf, wpa3, akm fields for PMF detection and WPA3 support.
    '''
    ...
    def __init__(self, fields):
        ...
        # --- V3: Security capability fields ---
        # pmf: Protected Management Frames (802.11w) detected via RSN IE
        self.pmf = False
        # wpa3: WPA3/SAE detected via AKM suite in RSN IE
        self.wpa3 = False
        # akm: Authenticated Key Management suite ('PSK', 'SAE', 'OWE', 'FILS', 'UNKNOWN')
        self.akm = 'PSK'
        # pmf_required: PMF required (not just capable) — deauth is completely useless
    ...
    def to_str(self, show_bssid=False):
        ...
        # V3: PMF and WPA3 indicators
        if self.pmf_required:
            pmf_str = Color.s('{R}REQ')
        elif self.pmf:
            pmf_str = Color.s('{O}yes')
        else:
            pmf_str = Color.s('{G} no')

        if self.wpa3:
            sec_str = Color.s('{C}WPA3')
        else:
            sec_str = Color.s('{W}    ')

        result = '%s  %s%s  %s  %s  %s  %s  %s  %s' % (
            essid, bssid, channel, encryption, power, wps, clients, pmf_str, sec_str)
        result += Color.s('{W}')
        return result
