...
class Arguments(object):
    ...
    def get_arguments(self):
        ...
        self._add_wpa3_args(parser.add_argument_group(Color.s('{C}WPA3{W}')))
        self._add_report_args(parser.add_argument_group(Color.s('{C}REPORTING{W}')))
    ...
    def _add_wpa3_args(self, wpa3):
        '''V3: WPA3/SAE attack options'''
        wpa3.add_argument('--wpa3',
            action='store_true',
            default=True,
            dest='use_wpa3',
            help=Color.s('Enable WPA3/SAE attack module (default: {G}on{W})'))

        wpa3.add_argument('--no-wpa3',
            action='store_false',
            dest='use_wpa3',
            help=Color.s('Disable WPA3/SAE attack module'))

        wpa3.add_argument('--wpa3-timeout',
            action='store',
            dest='wpa3_timeout',
            metavar='[seconds]',
            type=int,
            help=Color.s('Seconds to wait for WPA3 SAE capture (default: {G}120{W})'))


    def _add_report_args(self, rep):
        '''V3: Report generation options'''
        rep.add_argument('--report',
            action='store',
            dest='report_format',
            metavar='[html|json|md]',
            choices=['html', 'json', 'md'],
            help=Color.s('Generate audit report after session ({G}html{W}, {G}json{W}, or {G}md{W})'))

        rep.add_argument('--report-path',
            action='store',
            dest='report_path',
            metavar='[path]',
            type=str,
            help=Color.s('Output path prefix for report files (default: {G}wifite_report{W})'))
